...
interface CartridgeAccessors {
  ...
  /**
   * Writes the given unsigned byte at the cartridge's current offset, and then increments the
   * offset.
   *
  ...
  /**
   * Reads the short at the cartridge's current offset, and then increments the offset by {@link
   * Short#BYTES}.
   *
   * @return the short value at the current offset
   * @throws IndexOutOfBoundsException if {@link Cartridge#remaining()} is less than {@link
   *     Short#BYTES}.
   */
  short readShort();
  ...
  /**
   * Writes the given short at the cartridge's current offset, and then increments the offset by
   * {@link Short#BYTES}.
   *
  ...
  /**
   * Writes the given unsigned short at the cartridge's current offset, and then increments the
   * offset by {@link Short#BYTES}.
   *
  ...
  /**
   * Reads the int at the cartridge's current offset, and then increments the offset by {@link
   * Integer#BYTES}.
   *
  ...
  /**
   * Reads the unsigned int at the cartridge's current offset, and then increments the offset by
   * {@link Integer#BYTES}.
   *
  ...
  /**
   * Writes the given int at the cartridge's current offset, and then increments the offset by
   * {@link Integer#BYTES}.
   *
  ...
  /**
   * Writes the given unsigned int at the cartridge's current offset, and then increments the offset
   * by {@link Integer#BYTES}.
   *
  ...
  /**
   * Reads the long at the cartridge's current offset, and then increments the offset by {@link
   * Long#BYTES}.
   *
  ...
  /**
   * Writes the given long at the cartridge's current offset, and then increments the offset by
   * {@link Long#BYTES}.
   *
  ...
  /**
   * Transfers bytes from the cartridge into the given array, beginning at the cartridge's current
   * offset, which is then incremented by {@code dest.length}.
   *
  ...
  /**
   * Transfers {@code length} bytes from the cartridge into the given array, beginning at the
   * cartridge's current offset and at the given offset in the array. The cartridge's offset is then
   * incremented by {@code length}.
   *
   * @param dest the destination array
   * @param destOffset the offset within the array of the first byte to be written
   * @param length the number of bytes to be written
   * @throws IndexOutOfBoundsException if {@code destOffset} is out of bounds, i.e. less than 0
   *     or greater than {@code dest.length - destOffset}; or {@link Cartridge#remaining()} is less
   *     than {@code length}
   */
  void readBytes(final byte[] dest, final int destOffset, final int length);

  /**
   * Transfers bytes from the cartridge into the given array, beginning at the given offset in the
   * cartridge.
   *
  ...
  /**
   * Transfers {@code length} bytes from the cartridge into the given array, beginning at the given
   * offset in the cartridge and at the given offset in the array.
   *
   * @param offset the offset in the cartridge from which the first byte will be read
   * @param dest the destination array
   * @param destOffset the offset within the array of the first byte to be written
   ...
   *     greater than {@code ({@link Cartridge#size()} - length)}; or {@code destOffset} is out of
   *     bounds, i.e. less than 0 or greater than {@code dest.length - length}; or {@link
  ...
  /**
   * Transfers the entire content of the given array into the cartridge, beginning at the
   * cartridge's current offset, which is then incremented by {@code source.length}.
   *
  ...
  /**
   * Transfers {@code length} bytes from the given array into the cartridge, beginning at the
   * cartridge's current offset and at the given offset in the array. The cartridge's offset is then
   * incremented by {@code length}.
   *
   * @param source the source array
   ...
   * @param length the number of bytes to be written
   * @throws IndexOutOfBoundsException if {@code sourceOffset} is out of bounds, i.e. less than
   *     0 or greater than {@code (source.length - sourceOffset)}; or {@link Cartridge#remaining()}
   *     is less than {@code length}
   */
  void writeBytes(final byte[] source, final int sourceOffset, final int length);

  /**
   * Transfers bytes from the given array into the cartridge, beginning at the given offset in the
   * cartridge.
   *
  ...
  /**
   * Transfers {@code length} bytes from the given array into the cartridge, beginning at the given
   * offset in the cartridge and at the given offset in the array.
   *
   * @param offset the offset in the cartridge at which the first byte will be written
   * @param source the source array
   ...
   * @param length the number of bytes to be written
   * @throws IndexOutOfBoundsException if {@code offset} is out of bounds, i.e. less than 0 or
   *     greater than {@code ({@link Cartridge#size()} - length)}; or {@code sourceOffset} is out of
   *     bounds, i.e. less than 0 or greater than {@code (source.length - length)}; or {@link
   *     Cartridge#remaining()} is less than {@code length}
   */
  ...
  /**
   * Decodes {@code length} bytes to characters using the given charset, beginning at the
   * cartridge's current offset, which is then incremented by {@code length}.
   *
  ...
  /**
   * Decodes {@code length} bytes to characters using the given charset, beginning at the given
   * offset in the cartridge.
   *
   * @param offset the offset in the cartridge from which the first byte will be read
   * @param length the number of bytes to be read
   * @param charset the charset to be used
  ...
  /**
   * Writes the encoded characters using the given charset, beginning at the cartridge's current
   * offset, which is then incremented by the number of written bytes.
   *
  ...
  /**
   * Writes the encoded characters using the given charset, beginning at the given offset in the
   * cartridge.
   *
   * @param offset the offset in the cartridge at which the first byte will be written
   * @param sequence the sequence of characters to be encoded
  ...
  /**
   * Writes the encoded characters using the {@link StandardCharsets#US_ASCII} charset, beginning at
   * the cartridge's current offset, which is then incremented by the number of written bytes.
   *
   * @param sequence the sequence of characters to be encoded
   * @return the number of bytes written to the cartridge
   * @throws IndexOutOfBoundsException if {@link Cartridge#remaining()} is less than the number
  ...
  /**
   * Writes the encoded characters using the {@link StandardCharsets#UTF_8} charset, beginning at
   * the cartridge's current offset, which is then incremented by the number of written bytes.
   *
   * @param sequence the sequence of characters to be encoded
   * @return the number of bytes written to the cartridge
